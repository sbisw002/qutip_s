...
t = np.linspace(0.0, 200, 8000) # Define time vector
...
def H2_coeff(tt, args=None):
    return 1*np.sqrt(kappa)*np.exp(-( (tt-t0) /np.sqrt(2)/Sgm  ) ** 2)/np.sqrt(2*np.pi*Sgm*Sgm)
#    return 0
...
sp.io.savemat('./L2_Sgm20.mat', mdict={'Fs': Fs,'t': t,'bdb': output.expect[0],'b0p': output.expect[1],'bNp': output.expect[2],'exp_F': output.expect[3]})
